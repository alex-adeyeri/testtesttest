...
resource "aws_vpc" "sample_vpc" {
  ...
  tags = {
    Name = "sample_vpc-${random_integer.random.id}"
  }
  lifecycle {
...
resource "aws_subnet" "sample_public_subnet" {
  ...
  tags = {
    Name = "sample_public_${count.index + 1}"
  }
}
...
resource "aws_subnet" "sample_private_subnet" {
  ...
  tags = {
    Name = "sample_private_${count.index + 1}"
  }
}

resource "aws_db_subnet_group" "sample_rds_subnetgroup" {
  count      = var.db_subnet_group == "true" ? 1 : 0
  name       = "sample_rds_subnetgroup"
  subnet_ids = aws_subnet.sample_private_subnet.*.id
  tags = {
    Name = "sample_rds_sng"
  }
}
...
resource "aws_default_route_table" "sample_private_rt" {
  default_route_table_id = aws_vpc.sample_vpc.default_route_table_id
...
resource "aws_route_table_association" "sample_public_assoc" {
  count          = var.public_sn_count
  subnet_id      = aws_subnet.sample_public_subnet.*.id[count.index]
  route_table_id = aws_route_table.sample_public_rt.id
}
